> **Historical planning baseline / superseded status.** Preserved verbatim from teacher-edge main before the documentation split. M0–M3.0 completion text below reflects its original editing time. For current verified status use [STATUS](../STATUS.md); for next work use [ROADMAP](../ROADMAP.md).
